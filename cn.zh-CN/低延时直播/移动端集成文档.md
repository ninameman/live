...
* 修改ffmpeg编译脚本支持PCM解码（RtsSDK输出的是PCM数据）。/config/module-lite.sh 脚本中加入：
...
之后正常编译ffmpeg，不需要将Rts SDK依赖进去，参考ijk 的编译步骤。
...
需要将RTSSDK 库文件放入工程，修改Android.mk 等操作。参考拓展ffmpeg方法中的该步骤的详细介绍。
